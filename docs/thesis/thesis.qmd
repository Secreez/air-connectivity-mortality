...
Yes, combining them into a single **“Data Sources & Preparation”** section often reads more smoothly. It avoids the “double mention” of data sources in one subheading and the actual filtering in another. For a concise, academic style, you can merge them like this:
...
This thesis draws on two primary datasets and applies tailored filtering and aggregation steps to align them for correlation analysis:

1. **EUROCONTROL Air Traffic Data** [@eurocontrolData2025]:  
   - **Coverage:** December 2019 and March 2020, covering commercial flights from **Mainland China, Hong Kong, and Macao** to **EUROCONTROL member states**.  
   - **Preparation:**  
     - Filtered to include only flights originating in the three source regions.  
     - **Airport codes** matched using the **OurAirports dataset** [@ourairports2025] to ensure accurate country-level aggregation.  
     - Aggregated at the **destination country** level separately for **December 2019** (reflecting air traffic patterns prior to formal global recognition of COVID-19 [@chinazzi2020]) and **March 2020** (onset of European lockdowns).  
     - Result: A **country-level flight exposure ranking** based on inbound flights from the source regions.

2. **Excess Mortality Data** [@owidExcess2024; @karlinsky2021]:  
   - **Coverage:** All-cause mortality in European countries, drawn from Our World in Data (OWID), which compiles the **World Mortality Dataset (WMD)** and the **Human Mortality Database (HMD STMF)**.  
   - **Preparation:**  
     - Focuses on `excess_mortality_cumulative_per_million`, tracking the cumulative number of excess deaths per million inhabitants since **1 January 2020**, compared to a pre-pandemic baseline.  
     - Data were harmonized by **ISO3 country code** to ensure consistency with the flight data.  
     - **Four annual reference dates** were extracted to capture long-term pandemic outcomes:  
       - 05 May 2020  
       - 05 May 2021  
       - 05 May 2022  
       - 05 May 2023  

> *(Note: The choice of 05 May aligns with the WHO’s declaration ending COVID-19 as a Public Health Emergency of International Concern on **5 May 2023** [@who2023end]. Consistency across years was maintained by selecting 05 May in each preceding year.)*

By combining these two datasets, we obtain a comparative measure of **early flight exposure** and **excess mortality** at the **country level**, forming the basis for the subsequent correlation analysis.
...
Here, briefly summarizes the key findings of the results:
...
- **Mortality data quality:** Excess mortality estimates vary in completeness across countries due to differences in death registration systems.
