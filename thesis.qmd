...
| **P-score**            | Percent excess mortality = $(\text{Excess}/\text{Expected} − 1) × 100$                                                                               |
...
On **31 December 2019**, the Wuhan Municipal Health Commission reported a cluster of “viral pneumonia of unknown aetiology”; WHO issued its first Disease-Outbreak News five days later [@who2022timeline]. Europe’s first laboratory-confirmed SARS-CoV-2 infections were detected on **24 January 2020** in France [@spiteri2020]. By **21 February**, only 47 cases had been notified across nine countries—mostly imported—yet local transmission chains were already recognised in Bavaria and Haute-Savoie. Late that evening Italy reported the region’s first COVID-19 death and a fast-growing cluster in Lombardy [@ecdcitaly2020].

The six-week hop from Wuhan to Northern Italy shows how long-haul aviation can relocate a pathogen within a single generation interval. A global metapopulation model estimated that Wuhan’s **23 January** cordon sanitaire delayed local growth by only **3–5 days**, yet reduced *international* exportations by roughly **78 %** through mid-February [@chinazzi2020]. A review of ≈ 200 studies concludes that dense pre-2020 flight networks **shrank** Europe, enabling rapid cross-border seeding before the March–April capacity collapse [@sun2022_a].

Excess-mortality outcomes diverged sharply thereafter. Latent-class analyses for high-income countries indicate that government effectiveness, early and stringent NPIs, vaccine roll-out, and baseline life expectancy explain much of the longer-run gap [@rahmanian2024]. These factors speak to **amplification and suppression after importation**. Less clear is the role of **initial seeding**.
...
2. **Rank & correlate:** Compute Spearman *p* between exposure ranks and excess-mortality ranks on 5 May 2020; repeat for 2021–2023.
...
| **Sun 2022b**          | Sabre load-factor panel 2017–21        | 24 EU airlines               | **30–50 %** “ghost-flight” share in Mar–Apr 2020; EU slot waiver curtailed the practice.             |
| **Strohmeier 2021**    | OpenSky ADS-B (~46 % of 2019 flights) | Global coverage audit        | Community ADS-B misses large parts of East Asia → unsuitable for early-2020 China–EU counts.         |
| **Warnock-Smith 2021** | OAG seats + Sabre MIDT 2017–20         | CN/HK ↔ EU/Asia; CN domestic | Secondary CN–EU links collapsed to < 2 % of 2017 revenue; Chinese domestic LCCs rebounded first.     |
...
Crowd-sourced ADS-B (OpenSky) provides open historical data but with uneven geography—coverage is strongest in Europe/North America and sparse over East Asia [@iacus2020]. The OpenSky dataset paper estimates **~45–50 % global flight coverage** in 2019–2020 [@strohmeier2021]. In our coverage audit, OpenSky recorded **no** CN/HK arrivals for five EUROCONTROL states and **under-counted most others** relative to **ADRR** (Appendix @tbl-os-vs-eu-full; @fig-opensky-bar).
...
| **Statistic**         | Spearman’s $\rho$ with 95 % bootstrap CI (5,000 draws)                                                                  | Rank test robust to skew/outliers; bootstrap stabilises CI in a small-$n$ panel                          |
| **Sensitivity tests** | (i) **FPM** exposure; (ii) restrict to countries with complete 4-year $E_t$; (iii) partial $\rho$ controlling % ≥ 65 | Checks population scale, missing-data bias, and age confounding                                            |
...
| $E_t$            | excess deaths / 10⁶ on 5 May $t\       | $t = 2020,2021,2022,2023$ |

### Statistical approach

* **Association.** Spearman’s \$\rho\$ between each flight metric and \$E\_t\$.
...
| **Origin (ADEP)**      | Airport ISO-2 ∈ **CN** or **HK**                                  | Includes non-Wuhan mainland hubs that substituted after Jan 2020 [@chinazzi2020] |
...
“Ghost-flight” behaviour spiked in Mar–Apr 2020: **30–50 %** of EU departures flew with <10 % seats filled [@sun2022_b]. **ADRR counts movements, not load**—a three-passenger A350 counts the same as a full one. At Frankfurt, pax volume fell **–73 %** while freighters and “preighters” sustained **81 %** of cargo rotations [@fraport2021, p. 17]. Treat $F$ as **potential seeding capacity**, not realised traveller volume.
...
* **Source.** Country-day series from **Our World in Data’s COVID-19 dataset**, which republishes the **World Mortality Dataset** for excess deaths and OWID’s vaccination series [@karlinsky2021; @mathieu2021owidvaccinations].
* **Reference date.** For each year **2020–2023**, take the observation **nearest to 5 May** (± 7 days). **Ukraine is excluded ex ante** due to war-time breaks after 2021.
* **Why ± 7 days?** Most WMD feeds are weekly (some fortnightly). A ±½-week jitter is negligible relative to the **3–12 %** standard errors reported for high-income Europe and smaller than WHO model CIs [@msemburi2023].
...
*(For scale: by December 2020, CN–EU long-haul capacity was ~**7 %** of 2019 levels, while China’s domestic network had rebounded to ~**93 %** [@warnocksmith2021].)*
...
* **2020:** heavier direct CN/HK exposure ↔ **higher** first-wave excess mortality (*p* ≈ 0.53).
...
* **Reproducibility.** End-to-end R + Quarto (`build.sh`, `renv.lock`); 5,000-draw bootstrap CIs for every *p*.
