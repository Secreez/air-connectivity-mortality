...
A handful of gateways handled almost all traffic. Figure 1 (Top-10 bar chart) shows that Germany, the UK and the Netherlands alone handled > 60 % of all direct CN/HK arrivals in December 2019 and remained the top three even after the March collapse.
Interpretation: import pressure was highly concentrated in a few Western hubs.

Volume tracks route diversity. Figure 2 (origin-diversity panel) and Figure 3 (log-volume × diversity scatter) confirm a strong positive relationship (airport-level Spearman ρ ≈ 0.86, p < 0.001). Each extra ~100 flights added one-to-two new source airports.
> Outlier note – **Liège (LGG)** sits well above the trend because it is a freight gateway, so its flight count over-states passenger exposure (see [Data Limitations](#data-limitation) for details).

> Take-away: Both the amount of traffic and the variety of source cities funneled into the same Western European gateways, amplifying first-wave seeding risk.
...
```{r, out.width="80%", fig.align='center', fig.cap="March 2020 saw a cliff-edge drop in CN/HK → EU flights."}
knitr::include_graphics("data/figures/pct_change_filtered.png")
```

By late March 2020 every country had cut ≥ 50 % of direct CN/HK traffic; 15 of 19 slashed > 90 %… (collapse_tbl appendix!)
