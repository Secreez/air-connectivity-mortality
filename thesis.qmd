...
> **Early exposure landscape.** Direct CN/HK flights funnelled into just a few Western gateways. @top10-eu-dest-cntry shows that DEU, GBR and NLD alone handled > 60 % of all arrivals in December 2019, and they kept that lead even after traffic collapsed in March. Those same hubs also drew from the *widest mix* of Chinese origins: London‑Heathrow was linked to 15, Paris‑CDG to 13, and Frankfurt to 12 source airports (@origins_per_destination). Finally, volume and diversity rose in lock‑step (Spearman ρ ≈ 0.86; @scatter_volume_vs_origins). **Liège (LGG)** is the one clear outlier high diversity for only ~200 flights because most of its rotations were freighters (see [Data Limitations](#data-limitations)).
...
Before the pandemic shock, direct CN/HK traffic into Europe was extremely skewed. @top10-eu-dest-cntry ranks the **Top-10 destination countries** and already shows that three Western hubs—**Germany, the UK, and the Netherlands—absorbed > 60 % of all arrivals** in December 2019 and still dominated after the March collapse.


```{r top10-eu-dest-cntry, out.width="80%", fig.align='center', fig.cap="Top-10 European destinations for direct CN/HK passenger flights in December 2019 (orange), March 2020 (blue), and combined (green). Bars are counts of filed IFR passenger plans."}
...
* **Figure @pct_change_filtered** (percentage‑change bar plot)  
...
```{r pct_change_filtered, out.width = "80%", fig.align = "center"}
...
* **14 / 21 cut ≥ 75 %**, underscoring how synchronised the shutdown was.
...
```{r tbl‑spearman‑2020, echo = FALSE}
tbl_2020 <- spearman_res |>
  filter(year == 2020) |>
  select(variable, rho, ci_lo, ci_hi, p, n) |>
...
snap20 <- corr_df |>
  ...
  arrange(total_inbound_flights_combined)
...
```{r fig_scatter_2020, echo = FALSE, fig.asp = 0.7, fig.cap = "Direct CN/HK flights (Dec 2019 + Mar m2020, log‑scale) versus excess deaths per million on 5 May 2020. Spearman $\\rho \\approx 0.53$ (*p*< 0.01)."}
print(p20)
```

**Reading the 2020 snapshot.**  
All four exposure metrics are positively associated with first‑wave excess mortality (Table @tbl-spearman-2020). Using the **combined Dec + Mar count**:
...
```{r fig-rho-timeseries, fig.cap="Rank correlation between early CN/HK flight exposure and excess mortality, 2020–2023.  The seeding signal vanishes within a year.", fig.asp=0.6}
p_rho
```

```{r fig-rho-caterpillar, fig.cap="95 % bootstrap CIs confirm that only the 2020 estimate differs significantly from zero.", fig.asp=0.6}
gg_cater
```

@rho-timeseries makes the time‑trend clear: **ρ collapses to ≈ 0 in 2021 and flips negative thereafter.**  The caterpillar plot (@rho-caterpillar) shows that only the 2020 coefficient’s CI excludes zero.
...
*Controlling for population share ≥ 65 changes ρ by ≤ 0.02 in any year (Table @tbl-sensitivity). The sign‑flip therefore cannot be explained by age structure.*
...
In 2020, countries in the top quartile of direct CN/HK flight exposure registered **≈ 55 % higher excess mortality** than the bottom quartile, and the rank-based association was moderate-to-strong (Spearman ρ = 0.526, 95 % CI 0.177 - 0.771, *p* = 0.007; Table @tbl-spearman-all).
...
### Full coverage table {#tbl-os-vs-eu-full}
...
### Absolute change in flights, Mar 2020 vs Dec 2019 {#tbl-collapse}
