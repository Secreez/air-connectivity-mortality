...
```{r top10_eu_dest_cntry, out.width="80%", fig.align='center', fig.cap="Top-10 European destinations for direct CN/HK passenger flights in December 2019 (orange), March 2020 (blue), and combined (green). Bars are counts of filed IFR passenger plans."}
...
```{r pct_change_filtered, out.width="80%", fig-cap="..", fig.align='center'}
...
```{r tbl_spearman_2020, echo = FALSE}
...
All four exposure metrics are positively associated with first‑wave excess mortality (Table @tbl_spearman_2020). Using the **combined Dec + Mar count**:
...
```{r rho_caterpillar, fig.cap="95 % bootstrap CIs confirm that only the 2020 estimate differs significantly from zero.", fig.asp=0.6}
...
*Controlling for population share ≥ 65 changes ρ by ≤ 0.02 in any year (Table @tbl_sensitivity). The sign‑flip therefore cannot be explained by age structure.*
...
### Absolute change in flights, Mar 2020 vs Dec 2019 {#collapse}

```{r tbl_collapse, echo = FALSE}
knitr::kable(
  collapse_tbl |>
    dplyr::select(
      Country,
