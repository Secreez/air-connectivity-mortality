...
|-----------------------|------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------|
| **Unit of analysis**  | Country (n = 26 in 2020; n = 25 from 2022, when UKR is dropped)                                  | Islands & micro-states removed for missing flight **or** mortality data; Ukraine exits after war‐time breaks |
...
The final analysis panel is fully balanced: all 26 continental EUROCONTROL states have complete flight-exposure and excess-mortality data for each of the four reference years (2020 – 2023); no cells are missing.
...
We **keep the 21 EUROCONTROL states** that logged **≥ 5 direct CN/HK flights in Dec 2019**; anything smaller (e.g. Croatia’s single inbound charter) would turn a –100 % bar into noise yet epidemiologically trivial.
...
All four exposure metrics are **positively** associated with first-wave excess mortality (Table @tbl-spearman_2020). Three are statistically significant; the population-scaled metric is positive but not significant. Using the **Dec+Mar combined** count:
...
* Figure @fig-scatter-2020 shows the right-hand tail driven by the big Western gateways (**DEU, GBR, NLD, FRA, ESP, ITA**).
...
Controlling for the share aged ≥ 65 changes ρ by **≤ 0.02** in any year (Table @tbl-sensitivity), so age structure cannot explain the sign-flip.
...
Among the continental EUROCONTROL states, heavier direct **CN/HK → EU** traffic was associated with **substantially higher first-wave excess mortality** (Spearman ρ = **0.53**, *p* = **0.007**, *n* = **25**).
...
1. **Patchy ADS-B coverage.** A quick OpenSky audit showed East-Asian receivers captured far too few China-origin flights in **Feb 2020** (Appendix Table @tbl-os-vs-eu-full; Fig. @fig-opensky-bar).
