...
| **ρ (rho)**            | Spearman rank-correlation coefficient (association between two ranked variables)                                                                     |
...
| **Sun 2022b**          | Sabre load-factor panel 2017–21                   | 24 European airlines         | Airline-level shares of statistically “abnormal” low-load flights spiked in **Mar–Apr 2020** and waned by late-2021; timing aligns with **EU slot-relief (30 Mar 2020)**. |
| **Strohmeier 2021**    | OpenSky ADS-B (≈45% of FR24-tracked 2019 flights) | Global coverage audit        | Community ADS-B under-represents East Asia → unsuitable as a sole source for CN/HK→EU counts in early 2020.                                                               |
| **Warnock-Smith 2021** | OAG seats + Sabre MIDT 2017–20                    | CN/HK ↔ EU/Asia; CN domestic | By **Dec 2020**, China–Europe seat capacity ≈ **7%** of 2019 while China domestic ≈ **93%**; international markets lagged; Chinese domestic rebounded first.              |
...
Community ADS-B coverage is uneven. OpenSky’s 2019 archive lists **≈31.0 million** flights—**≈45%** of the **68.95 million** flights Flightradar24 tracked that year—with much stronger reception over Western Europe than East Asia; because FR24 also uses non-ADS-B sources, **45%** is an **upper-bound** comparison. [@strohmeier2021]

For early-2020 **CN/HK → Europe** links, this bias matters: in our audit OpenSky records **zero** arrivals for five EUROCONTROL states and **under-counts** most others relative to EUROCONTROL. (Details in Appendix @tbl-os-vs-eu-full; @fig-opensky-bar.)

By contrast, EUROCONTROL’s *Aviation Data Repository for Research* (**ADRR**) contains **every filed IFR movement touching European airspace**, independent of ADS-B reception. Public snapshots exist for **Mar | Jun | Sep | Dec**; **Dec 2019** and **Mar 2020** bracket the pre-lockdown seeding window used here.
...
| **Origin (ADEP)**      | Airport ISO-2 ∈ **CN** or **HK**                                  | Includes non-Wuhan hubs that substituted from mid-Jan 2020. [@chinazzi2020]     |
| **Destination (ADES)** | Airport lies in the **25 continental EUROCONTROL states**         | Aligns exposure with the mortality panel                                         |
| **De-duplication**     | Collapse identical *callsign + off-block time* rows               | Removes re-filed plans; yields unique movements                                  |
| **Metadata sanity**    | Drop rows missing ADEP/ADES/flight-type                           | Prevents merge errors and mis-counts                                             |
...
OpenSky captured **≈45%** of FR24’s 2019 flight total and is geographically skewed toward Europe, so raw ADS-B **under-states** **CN/HK → EU** exposure, including **zeros** for several states in our window. **ADRR** records **all filed IFR**, recovering movements missed by ADS-B and providing a more reliable exposure baseline. [@strohmeier2021]
(See **Coverage audit** below.)
...
**Purpose.** Test whether community ADS-B (OpenSky) under-counts **direct CN/HK → EUROCONTROL** flights in **Dec 2019 / Feb 2020 / Mar 2020**.
...
> **Early exposure landscape.** Direct CN/HK flights funneled into a handful of Western European gateways. @fig-top10_eu_dest_cntry shows **Germany, the UK, and the Netherlands** handling a majority of arrivals in December 2019 and retaining that lead after the March collapse. Those same hubs also drew from the **widest mix** of Chinese origins—**LHR 15**, **CDG 13**, **FRA 12** (@fig-origins_per_destination). Volume and diversity rose in lock-step (airport-level Spearman **ρ ≈ 0.86**; @fig-scatter_volume_vs_origin). **Liège (LGG)** sits above the trend—high origin diversity for ~200 rotations—because many S/N movements there are **cargo charters** (see [Limitations](#limitations)).
...
Before the pandemic shock, direct CN/HK traffic into Europe was highly skewed. @fig-top10_eu_dest_cntry (Top-10 destination **countries**) shows **Germany, the UK, and the Netherlands** absorbing over half of all arrivals in December 2019 and still dominating after the [March cliff-edge](#march-cliff).
...
With direct CN/HK traffic reduced to a trickle after late March, any variation in **first-wave** excess mortality must trace back to flights that arrived **before** the cliff-edge. The next subsection tests whether pre-collapse exposure leaves a statistical imprint in the **5 May 2020** snapshot. Though, OAG covers scheduled passenger capacity; charter flights are excluded and cargo flights are not included—so these figures are a capacity baseline, not movements or passengers. [@warnocksmith2021]
...
```{r fig-scatter-2020, echo = FALSE, fig.asp = 0.7, fig.cap = fig.cap ="Direct CN/HK S/N flights (Dec-2019 + Mar-2020, log-scale) versus excess deaths per million on 5 May 2020 (n = 25). Spearman $\rho \approx 0.53$ (*p* < 0.01). Several Western hubs anchor the right tail; **ESP** is a high-mortality outlier at moderate exposure."}
...
* **Vaccination rollout.** By **5 May 2022** (±7 days), the **highest-exposure quartile (Q4)** had **higher full-vaccination** than lower-exposure quartiles (see Table @tbl-vaccination_exposure_res), aligning with OWID’s series and helping explain the **post-2020 attenuation/flip**. [@mathieu2021owidvaccinations]
...
* **Lean, transparent cleaning.** We keep **S/N** traffic only, de-duplicate exact re-files, and publish scripts/derived tables alongside the manuscript. [@eurocontrol2022seg]
* **Reproducibility.** End-to-end R + Quarto with `build.sh` and `renv.lock`; **5,000-draw bootstrap CIs** accompany every Spearman $\rho$.

## Limitations {#limitations}

* **Load-factor bias.** Many EU carriers operated **statistically “abnormal” (very low-load)** flights in **Mar–Apr 2020**, receding through 2021; “abnormal” is defined **statistically** (IQR-based outliers vs 2017–2019 baselines), **not** as “<10% seats”. Hence $F_{\text{SUM}}$ measures **potential seeding capacity**, not realised traveller volumes. [@sun2022_b] (At a major hub like **Frankfurt**, passenger volumes collapsed while cargo shifted to freighters and “preighters”.) [@fraport2021]
* **February gap.** No public **ADRR** slice for **Feb 2020** → peak early-exposure may be **under-estimated**.
* **Indirect legs invisible.** Multi-leg routings (e.g., **PVG→DXB→VIE**) and non-air links aren’t captured by direct-flight counts.
...
The project began building a global **effective-distance** SIR model [@brockmann2013], but two constraints forced a pivot: **(i)** sparse **East-Asia ADS-B** coverage in early 2020 (plus OpenSky’s flight-derivation quirks) [@strohmeier2021], and **(ii)** no **February** ADRR snapshot (public months are **Mar | Jun | Sep | Dec**). Given those constraints—and a BA time-box—we adopted an exploratory, fully reproducible **rank-correlation** design on **Dec 2019** and **Mar 2020** exposures against **four annual** excess-mortality snapshots.
...
OpenSky’s community ADS-B coverage in early 2020 is uneven—**strong in Western Europe**, **thin over East Asia** and parts of **Eastern Europe**—so it **under-counts** direct CN/HK arrivals relative to EUROCONTROL ADRR. In our audit, OpenSky **misses** five countries entirely and **under-counts** most others; we therefore treat OpenSky as a **QC benchmark only**. [@strohmeier2021]
