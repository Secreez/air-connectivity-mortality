...
The six-week hop from Wuhan to Northern Italy illustrates how long-haul aviation can relocate a pathogen within a single incubation cycle. A global metapopulation model by **@chinazzi2020** estimated that Wuhan’s **23 January** cordon sanitaire delayed local growth by only **3–5 days**, yet still cut *international* exportations by roughly **78 %** through mid-February [@chinazzi2020]. A narrative review of ≈ 200 studies concludes that dense pre-2020 flight networks “shrank” Europe, enabling rapid cross-border seeding before March–April capacity collapses [@sun2022_a].

Excess-mortality outcomes, however, diverged sharply thereafter. Latent-class analyses for high-income countries show that *domestic* factors—government effectiveness, early and stringent NPIs, vaccine roll-out, and baseline life expectancy—best explain the longer-run gaps [@rahmanian2024].  

What remains unknown is how much of the first-wave mortality spread can be attributed to a country’s direct-flight exposure to **mainland China and its two Special Administrative Regions (Hong Kong and Macao)** before lockdowns. This thesis addresses that descriptive gap.

## Research Question

> Among continental EUROCONTROL member states, is a higher volume of direct inbound flights from China/Hong Kong/Macao (Dec 2019 & Mar 2020) associated with higher cumulative excess mortality on 5 May 2020?^[Although Macao was initially included, EUROCONTROL's dataset showed no direct flights from Macao to Europe in the study period, effectively limiting analysis to Mainland China and Hong Kong (see Chapter 3).]
...
| **Warnock-Smith 2021** | OAG seats + Sabre MIDT revenue 2017-20 | Mainland China/Hong Kong ↔ EU/Asia, China domestic | Domestic LCCs rebounded first; secondary China-EU links collapsed to < 2 % of 2017 revenue. |

*Table 1. Landmark air-traffic studies that inform this thesis*

Together, these studies establish that long-haul links seeded the virus early, while later traffic collapses and “ghost flights” distort raw flight counts precisely the measurement limitation highlighted
...
All three studies, however, start their analysis after SARS-CoV-2 had already seeded domestically. This thesis examines one step earlier: whether countries with heavier direct China/Hong Kong air traffic experienced larger initial import shocks, potentially shaping subsequent mortality trajectories.
...
Crowd-sourced ADS-B feeds such as **OpenSky** are a tempting shortcut for quick global aviation analytics, yet their **sensor network was patchy over East Asia in early 2020**. Independent audits find that barely half of all China-origin flights were visible in the raw feed [@strohmeier2021; @iacus2020].

Our own cross-check (Table [@tbl-os-vs-eu]) confirms the blind-spot problem: **five EUROCONTROL member-states receive no China/HK arrivals in OpenSky at all**, and most low-volume countries show double-digit under-counts for December 2019.

EUROCONTROL’s *Aviation Data Repository for Research* (ADRR), by contrast, logs **every filed IFR movement that touches European airspace**, irrespective of ADS-B reception. Although ADRR is released for only four months each year, the public snapshots for **December 2019** and **March 2020** cover the two key pre-epidemic months required here.

Consequently, **all exposure metrics in this thesis are based on EUROCONTROL**. OpenSky is kept **solely as a quality-control benchmark** to illustrate why ADS-B alone would have understated early China-Europe connectivity (see Appendix A).
...
The EUROCONTROL schedule includes traffic from mainland China (CN), Hong Kong (HK).
...
OpenSky’s crowd-sourced ADS-B feed is often used for rapid aviation analyses, but its sensor density over East Asia remained sparse in early 2020 [@strohmeier2021].

Table @tbl-os-vs-eu lists, for each EUROCONTROL state, the direct CN/HK flights
captured by both sources in Dec 2019, Feb 2020, and Mar 2020.

```{r tbl-os-vs-eu, echo=FALSE}
opensky   <- readr::read_rds("data/processed/opensky/cn2eu_wide.rds")
euro_cntr <- readr::read_rds("data/processed/flights_country.rds") |>
  dplyr::select(
    iso2       = iso_country,
...
euro_map  <- readr::read_csv("data/eurocontrol_iso_map.csv",
                             show_col_types = FALSE)

euro_wide <- euro_cntr |>
  dplyr::left_join(euro_map, by = "iso2") |>
  dplyr::select(iso3, Dec19_eu, Mar20_eu)
...
coverage_tbl <- opensky |>
  dplyr::rename(
    Dec19_os = `2019-12`,
    Feb20_os = `2020-02`,
    Mar20_os = `2020-03`
  ) |>
  dplyr::left_join(euro_wide, by = "iso3") |>
  dplyr::mutate(
  ...
  dplyr::arrange(dplyr::desc(Dec19_eu))

coverage_tbl |>
  dplyr::select(
    iso3,
    `Dec 2019 (EU)` = Dec19_eu,  `Dec 2019 (OS)` = Dec19_os,
    `Feb 2020 (OS)` = Feb20_os,
    `Mar 2020 (EU)` = Mar20_eu,  `Mar 2020 (OS)` = Mar20_os
  ) |>
  knitr::kable(
    caption = "Direct CN/HK flights captured by EUROCONTROL (EU) and OpenSky (OS)."
  )

```

OpenSky **omits GEO, HRV, PRT, SVK, and UKR entirely** and reports ≥ 1 flight fewer than EUROCONTROL for 14 of the remaining 21 states (median deficit ≈ 12 flights). In short, using raw ADS-B counts would have **systematically understated exposure for peripheral and low-traffic markets—**the very cases that matter for a rank-based analysis. For that reason the study proceeds with EUROCONTROL data and treats OpenSky only as a sanity check (see Appendix A).
...
A.1 Grouped bars: December 2019 and February 2020 counts from Table [@tbl-os-vs-eu] visualised side-by-side.
