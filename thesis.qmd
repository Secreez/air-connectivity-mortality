---
title: "Direct China/Hong Kong flight exposure and excess mortality in Europe’s first COVID-19 wave: an exploratory analysis using EUROCONTROL ADRR"
author:
...
Table: Key acronyms and symbols used in this thesis. {#tbl-glossary}
...
What remains unknown is how much of the first-wave mortality spread can be associated with a country’s direct-flight exposure to **mainland China and its two Special Administrative Regions (Hong Kong and Macao)** before lockdowns. This thesis addresses that empirical gap in descriptive terms.

## Research Question

> Among EUROCONTROL member states, is a higher volume of direct inbound flights from China/Hong Kong/Macao (Dec 2019 & Mar 2020) associated with higher cumulative excess mortality on 5 May 2020?^[Macao appears in the original acronym CN/HK/MO, but the EUROCONTROL snapshots used here show **zero** Macao-Europe flights in Dec 2019 or Mar 2020. Data handling details are given in [Data & Methods](#methods)].
...
Recent syntheses—most recently @pizzato2024 maps the socio-economic and vaccination gradients in Europe’s excess mortality, but they give scant attention to **importation pathways**. This thesis adds an early-mobility lens.
...
Our own test (coverage audit) shows that OpenSky records **no** CN/HK arrivals for five EUROCONTROL member states and under-counts most others.
...
Because completeness is critical here, **all exposure metrics use ADRR**; OpenSky is retained only as a **sanity-check benchmark**, see [Coverage audit](#coverage-audit) and @tbl-os-vs-eu-full within the Appendix.

# Data & Methods {#methods}

EUROCONTROL’s public **ADRR** snapshots record every commercial IFR movement that touches European air-space.  
For this study we extract flights whose **origin** is any airport in **mainland China (CN)** or **Hong Kong (HK)** and whose **destination** is one of the 25 EUROCONTROL member states. (No Macao–Europe flights appear in either snapshot, so exposure is effectively CN + HK; the full acronym *CN/HK/MO* is retained only on first mention.)
...
|-----------------------|------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------|
| **Unit of analysis**  | Country (**n = 25** throughout; **Ukraine excluded ex ante**)                                  | Islands & micro-states removed for missing flight **or** mortality data; Ukraine exits after war‐time breaks |
...
| **Outcome**           | $E_t =$ excess deaths / 10⁶ on 5 May of year $t$ ($t = 2020–2023$)                          | Fixes a uniform “first-wave” reference point; OWID/WMD snapshot                        |
...
We did not model vaccination, NPIs, GDP, or other covariates; any references to these in the Discussion are descriptive context rather than part of the statistical tests.
...
is not publicly available; rerouting via non-Wuhan hubs had begun well before [@chinazzi2020].
...
| **Destination (ADES)** | Airport lies in the **25 EUROCONTROL member states**                                              | States that lack either direct CN/HK traffic *or* reliable mortality data                                    |
...
$$
F_{\text{DEC}} = \text{number of CN/HK \rightarrow EU flights in Dec 2019},\qquad
F_{\text{MAR}} = \text{number of flights in Mar 2020},\qquad
...
*(Full R pipeline: `R/`; processed tables in `data/processed/`.)*

## Coverage audit - EUROCONTROL vs OpenSky {#coverage-audit}

OpenSky’s crowd-sourced ADS-B feed was thin over **East Asia and parts of Eastern Europe** in early 2020.

* **Misses** \texttt{GEO, HRV, PRT, SVK, UKR} entirely  
* **Under-counts** 14 of those 21 states (median deficit ≈ 12)

Full details are in @tbl-os-vs-eu-full within the Appendix.
...
coverage_tbl |>
  ...
  head(20) |>
  knitr::kable(
    caption = "Excerpt of the Coverage audit (top half of the table). All 41 rows are reproduced in @tbl-os-vs-eu-full within the Appendix"
  )
```
...
* **Source.** Excess-mortality **and vaccination** variables come from the **Our World in Data COVID-19 dataset** [@owid_covid2025], which republishes the *World Mortality Dataset* [@karlinsky2021] and the OWID vaccination series [@mathieu2021owidvaccinations].

* **Reference date.** For each calendar year we take the observation **nearest to 5 May** (± 7 days). Ukraine is excluded after 2021 because war disrupts reporting.
...
* Comparing OWID to raw UN WPP (via **wpp2024** R package) for all EUROCONTROL member states:  
...
5. **Reproducibility** – Code, data and rendered Quarto reports are version-tracked in the project repo.

### Visual encoding & accessibility {#viz-note}

All plots use the **Okabe–Ito** colour-universal palette [@okabe2002] (`#009E73 green, #E69F00 orange, #56B4E9 blue …`). Axis titles, direct labels and dashed reference lines ensure figures remain legible in greyscale printing and for the main forms of colour-vision deficiency. Hues with strong cultural valence (e.g., alert-red) are avoided to minimise framing bias.
...
1. **Data-infrastructure awareness** Two complementary data streams are interrogated: crowd-sourced ADS-B (**OpenSky**) with patchy East-Asian coverage and no load factors, versus regulator-grade IFR plans (**EUROCONTROL ADRR**) with full spatial coverage but equally no passenger counts. Mapping the blind spots of each stream (see [Coverage audit](#coverage-audit)) is integral to the method, not an after-thought.

2. **Single-command rebuild** A bash wrapper `build.sh` orchestrates the complete pipeline *raw CSV → clean RDS → Quarto → final PDF/HTML Manuscript*. Running the script on a fresh clone regenerates every table and figure without manual edits, except the CSV.

3. **Pinned R environment** The repository carries an `renv.lock` snapshot. Restoring that lock guarantees the identical package stack used during development; in `R/00_load_libs.R` automatically installs any missing packages if the lock is absent.

4. **Path-safe, self-checking code** All scripts build paths with `here::here()`, so the project runs from any working directory. `build.sh` stops early with a clear message, if the large ADRR CSV dumps are missing.
...
**Transition.** With data quality, exposure/outcome definitions, and reproducibility safeguards in place, we now turn to the empirical results.
...
The final analysis panel is fully balanced: all 25 EUROCONTROL member states have complete flight-exposure and excess-mortality data for each of the four reference years (2020 – 2023); no cells are missing.

## Early exposure landscape

> **Early exposure landscape.** Direct CN/HK flights funnelled into just a few Western gateways. @fig-top10_eu_dest_cntry shows that **DEU, GBR and NLD handled more than 60 % of all arrivals** in December 2019 and retained that lead even after traffic collapsed in March. Those same hubs also drew from the *widest mix* of Chinese origins: London–Heathrow linked to 15, Paris–CDG to 13, and Frankfurt to 12 source airports (@fig-origins_per_destination). Finally, volume and diversity rose in lock‑step (Spearman ρ ≈ 0.86; @fig-scatter_volume_vs_origins). **Liège (LGG)** is the one clear outlier high diversity for only ~200 flights because most of its rotations were freighters (see [Data Limitations](#data-limitations)).

> *Together, these three plots show that both **how much** traffic and **how many** source cities were concentrated in the same Western hubs, setting up a right-skewed exposure profile for Europe’s countries.*

### A handful of gateways handled almost all traffic

Before the pandemic shock, direct CN/HK traffic into Europe was extremely skewed. @fig-top10_eu_dest_cntry ranks the **Top-10 destination countries** and already shows that three Western hubs—**Germany, the UK, and the Netherlands**—absorbed more than 60 % of all arrivals in December 2019 and still dominated after the March collapse.
...
> **Volume and diversity rise in tandem:** each extra ~100 flights brings one-to-two additional Chinese origin airports (airport-level Spearman ρ ≈ 0.86, p < 0.001, n = 47).
...
We **keep the 21 EUROCONTROL member states** that logged **≥ 5 direct CN/HK flights in Dec 2019**; anything smaller (e.g. Croatia’s single inbound charter) would turn a –100 % bar into noise yet epidemiologically trivial.

The same ≥ 5‑flight filter is used for:

* **@fig-pct_change_filtered** (percentage‑change bar plot)  
* **@tbl-collapse** (absolute Δ‑flights in the Appendix)

Subsequent correlation analyses use the full 26-country panel.
...
Among the **21 ≥ 5-flight countries**, every one cut traffic by **at least 50 %** by the end of March 2020:

* **7 of 21 slashed > 90 %**—the bars pinned hard against −100 %.
* **14 of 21 cut ≥ 75 %**—another sign of a tightly synchronised shutdown.
...
| **Residual risk** | Absolute flight counts still diverged after March: a handful of hubs kept 200‑350 rotations, so a trickle of import potential remained (see Δ-flights column in @tbl-collapse). |

With direct CN/HK traffic reduced to a trickle after late March, any variation in Europe’s **first-wave** excess mortality must trace back to flights that arrived **before** the cliff-edge. The next subsection therefore asks whether that pre-collapse exposure still leaves a statistical imprint in the excess-mortality snapshots taken on 5 May 2020.

*(For scale: by December 2020, CN–EU long-haul capacity had fallen to roughly **7 %** of 2019 levels, whereas China’s domestic network had already rebounded to about **93 %** [@warnocksmith2021].)*
...
* **Spearman ρ = 0.526** (95% CI **0.177–0.771**, *p* = 0.007).
...
```{r fig-scatter-2020, echo = FALSE, fig.asp = 0.7, fig.cap = "Direct CN/HK flights (Dec 2019 + Mar 2020, log-scale) versus excess deaths per million on 5 May 2020 (n = 25 countries). Spearman $\\rho \\approx 0.53$ (*p* < 0.01)."}
knitr::include_graphics("data/figures/scatter_2020.png")
```

### Does the signal persist? (2020 → 2023)

```{r fig-rho_caterpillar, fig.cap="Spearman ρ (points) with 95% bootstrap CIs (bars) for Dec 2019, Mar 2020 and their sum. Estimates are positive in 2020. From 2021 onward they are zero or negative; in 2022 the Mar and Combined metrics are significantly below zero. In 2023 they remain negative, with CIs that touch or narrowly include zero.", fig.asp=0.6}
knitr::include_graphics("data/figures/rho_caterpillar.png")
```

## Long-run trend (2020 → 2023)

**Pattern.**
The 2020 coefficients are significantly **positive**. The association weakens in 2021 and turns **negative** in 2022, with **Mar** and **Combined** significantly below zero (e.g., ρ ≈ –0.46, *p* ≈ 0.02). In **2023** the estimates stay negative but are **borderline** (the 95% CIs graze or narrowly include zero). Scaling by population (*Flights per million*) makes the flip appear **already by 2021**.
...
Controlling for the share aged ≥ 65 changes ρ by **≤ 0.02** in any year (@tbl-sensitivity), so age structure cannot explain the sign-flip.

### Key take-away — the signal flips within a year

* **2020:** **positive** — ρ = **0.526** (95% CI **0.177–0.771**, *p* = **0.007**).
* **2021:** near **zero/weakly negative** — ρ = **–0.306** (95% CI **–0.622–0.111**, *p* = **0.137**).
* **2022:** **negative and significant** — ρ = **–0.460** (95% CI **–0.792––0.001**, *p* = **0.021**).
* **2023:** **negative, borderline** — ρ = **–0.419** (95% CI **–0.749–0.045**, *p* = **0.037**; CI narrowly spans zero).
* **Flights per million (population-scaled):** already **negative and significant in 2021** — ρ = **–0.531** (95% CI **–0.787––0.126**, *p* = **0.006**); **2022:** ρ = **–0.621** (*p* = **0.001**); **2023:** ρ = **–0.570** (*p* = **0.003**).

## Main finding in one breath

Among the EUROCONTROL member states, heavier direct **CN/HK → EU** traffic was associated with **substantially higher first-wave excess mortality** (Spearman ρ = **0.53**, *p* = **0.007**, *n* = **25**).
The rank-based link weakened during 2021 and **reversed by 2022** (negative, significant), remaining **negative but borderline** in 2023.

## Why did the signal fade?

* **Wealth & structural vulnerability.** A 34-country panel shows excess-death percentages **fall** with GDP per capita and **rise** with poverty/inequality [@ioannidis2023]. Europe’s most connected hubs (DEU, GBR, NLD, FRA) sit in the top GDP tercile, helping explain why the positive flight–mortality link disappears after 2020.
* **Vaccination rollout.** On **5 May 2022** the high-exposure quartile (**Q4**, *n* = 6) had a median **78%** fully vaccinated versus **≈62%** in the low-exposure quartile (**Q1**, *n* = 7) (OWID `people_fully_vaccinated_per_hundred` [@mathieu2021owidvaccinations]).
...
* **Policy timing & capacity.** Countries that paired **high government effectiveness** with **early, stringent NPIs** cluster in the lowest excess-mortality band [@rahmanian2024]. Complementary aviation work shows that stand-alone travel bans are blunt unless combined with domestic measures and that the EU “slot waiver” quickly eliminated **40 % + low-load “ghost flights“** in April 2020 [@sun2022_a; @sun2022_b].

## Strengths

* **Regulator-grade data.** ADRR captures *every* IFR passenger movement; the East-Asia coverage gaps of ADS-B are avoided.  
* **Lean, transparent cleaning.** Only S/N codes kept, exact duplicates removed; raw CSVs and scripts live in the repo.  
* **Reproducibility.** End-to-end R + Quarto workflow (`build.sh`, `renv.lock`), 5 000-draw bootstrap CIs for every ρ.

## Limitations

* **Load-factor bias.** Up to 40 % of EU departures in March-April 2020 flew with < 10 % seats filled → `F_SUM` measures *potential* imports, not actual infected travellers.  
* **February gap.** No ADRR snapshot for Feb 2020 → peak exposure may be underestimated.  
* **Indirect legs invisible.** Multi-leg itineraries (e.g. PVG → DXB → VIE) remain uncounted.  
* **Ecological, small *n*.** Country-level design (~ 25 units) hides within-state variation; residual confounding by GDP, ICU capacity, NPI stringency is possible.

## Methodological reflections

The project **started** as an ambitious plan to run a global SEIR simulation on “effective-distance” air routes [@brockmann2013].  
Two hard data gaps forced a pivot:

1. **Patchy ADS-B coverage.** A quick OpenSky audit showed East-Asian receivers captured far too few China-origin flights in **Feb 2020** (Appendix @tbl-os-vs-eu-full; @fig-opensky-bar).
2. **No February ADRR slice.** EUROCONTROL publishes IFR snapshots only for **Mar | Jun | Sep | Dec** (see [Methods, Snapshot-Source](#snapshot-source)).

Given those constraints—and the bachelor-thesis time-box—we pivoted to a lean, fully reproducible **rank-correlation design** that uses the only complete pre-lockdown months available (**Dec 2019** and **Mar 2020**). The resulting exposure table is regulator-grade but still measures *potential* rather than passenger-weighted seeding, leaving passenger loads and multi-leg itineraries for future work.

## Implications & future work

**What the results do say.** Direct CN/HK flight volumes help explain cross-country differences in Europe’s **spring 2020** excess mortality. Beyond 2020, the association weakens and reverses as vaccination, health-system capacity and socioeconomic factors take over.

**What the results don’t say.** These are ecological, rank-based associations; they do not identify causal effects or passenger-level transmission.

**Avenues for future research (not undertaken here).**
1. **Passenger weighting.** Analyses would benefit from IATA/OAG passenger loads to reduce the influence of low-occupancy “ghost” flights.
2. **Multi-leg itineraries.** Incorporating inferred OD chains (e.g., PVG→DXB→VIE) and non-air links (maritime/land) would better capture true import pathways.
3. **Sub-national panels.** Moving from countries to regions or metros could reduce ecological bias and align exposure with local mortality.
4. **Integrated models.** Network SEIR/SEnIR studies could combine exposure, NPIs, vaccination and health-system capacity once suitable mobility and health data layers are available.

**Bottom line.** Flight counts are a useful proxy for **initial import pressure**; explaining outcomes after 2020 requires richer passenger and policy data.
...
## Data-source triage & supplementary plots {#appendix-a}

### Full coverage table

```{r tbl-os-vs-eu-full, echo=FALSE, tbl-cap="Direct CN/HK flights captured by EUROCONTROL (EU) versus OpenSky (OS). Rows sorted by EUROCONTROL’s December-2019 counts."}
knitr::kable(
  coverage_tbl,
  caption = NULL
)
...
```{r tbl-collapse, echo=FALSE, tbl-cap="Absolute change in direct CN/HK → EUROCONTROL flights between December 2019 and March 2020 (countries with ≥ 5 December flights). Residual March counts highlight hubs that still handled 200–350 rotations after the policy cliff-edge."}
knitr::kable(
  collapse_tbl |>
...
![Top-10 destinations by number of direct CN/HK flights in OpenSky data (Dec 2019, Feb 2020, Mar 2020). OpenSky reflects community ADS-B coverage, not filed IFR plans; the grouped bars show erratic coverage patterns and motivate prioritizing EUROCONTROL.](data/figures/bar_opensky_top10_dec_feb_mar.png){#fig-opensky-bar width="90%"}

### Vaccination by exposure quartile (May 2022)

```{r tbl-vaccination-quartiles, echo=FALSE, tbl-cap="Country membership of the flight-exposure quartiles (Dec 2019 + Mar 2020 baseline, *n* = 25; quartiles contain 6–7 countries each)."}
corr_df |>
  dplyr::filter(target_date == as.Date("2020-05-05")) |>
  ...
  knitr::kable(col.names = c("Q1 (low)", "Q2", "Q3", "Q4 (high)"), caption = NULL)
```

*For the descriptive comparison in the Discussion, we extracted `people_fully_vaccinated_per_hundred` from the OWID series, taking the observation nearest to **5 May 2022 (± 7 days)** for the same EUROCONTROL set (`01b_extract_vax.R` → `data/processed/vax_snapshot.{csv,rds}`). These figures are **not** used in any regression or correlation.*
