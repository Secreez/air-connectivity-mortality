...
EUROCONTROL’s public **ADRR** snapshots record every commercial IFR movement touching European airspace.  
For this study we extract flights whose **origin** is any airport in **mainland China (CN)** or **Hong Kong (HK)** and whose **destination** is one of the 26 continental EUROCONTROL member states. (No Macao–Europe flights appear in the snapshots, so exposure is effectively CN + HK; the full acronym *CN/HK/MO* is kept only on first mention.)

Excess-mortality outcomes come from **OWID’s** country-day series, which republishes the *World Mortality Dataset*.  For each year 2020-2023 we take the observation **closest to 5 May** (≤ ± 7 days).

*Why 5 May?* It captures the tail of Europe’s first epidemic wave and—fortuitously—marks the WHO announcement (5 May 2023) that COVID-19 was no longer a global public-health emergency [@who2023end].
...
|-----------|--------|-----------|
| **Unit of analysis** | Country (n = 26) | Islands & micro-states excluded for missing flight or mortality data |
| **Exposure** | Flight counts in **Dec 2019** (*F*<sub>DEC</sub>) and **Mar 2020** (*F*<sub>MAR</sub>); summed as *F*<sub>SUM</sub> | Brackets the entire January–February seeding window while preceding most EU lockdowns |
| **Outcome** | *E*<sub>t</sub> = excess deaths per million on 5 May of year *t* (t = 2020…2023) | Snapshot avoids later seasonal waves; values from OWID/WMD |
| **Statistic** | Spearman ρ with 5 000-draw bootstrap 95 % CI | Rank test robust to skew/outliers; bootstrap stabilises CI in small-n setting |


### Flight-exposure variable

**Source.** EUROCONTROL’s *Aviation Data Repository for Research* (**ADRR**) publishes full IFR flight plans for four “snapshot” months each year (Mar | Jun | Sep | Dec).

For early‐COVID seeding only the **Dec-2019** and **Mar-2020** slices are available; February sits in the blind spot but, as @chinazzi2020 show, rerouting through non-Wuhan hubs began well before February, so December already captures the relevant network. Mar-2020, taken after Wuhan’s shutdown but just before most European lockdowns, shows the residual links.

**Filters applied**

| Step                   | Rule                                                | Rationale                                                              |
| ---------------------- | --------------------------------------------------- | ---------------------------------------------------------------------- |
| Traffic type           | ICAO *S* (scheduled) or *N* (non-scheduled charter) | Removes cargo-only *F* and general-aviation *G* plans                  |
| Origin (**ADEP**)      | Any airport in **CN** or **HK**                     | Includes all hubs that substituted for Wuhan (e.g. PVG, PEK, CAN, HKG) |
| Destination (**ADES**) | One of the **26** continental EUROCONTROL states    | Islands & micro-states excluded elsewhere for missing outcome data     |
| De-duplication         | Same callsign + same wheels-off time collapsed      | Removes flight-plan re-filings                                         |

> **Load-factor caveat.** Slot-retention “ghost flights” peaked in Mar-Apr 2020; @sun2022_b estimate that **30–50 %** of EU departures in that window flew with < 10 % seats filled. ADRR records the movement but not the payload, so a three-passenger A350 counts the same as a full one. At Frankfurt, for example, passenger volume collapsed **–73 %** while freighters + “preighters” kept **81 %** of cargo rotations alive [@fraport2021, p. 17]. Interpret flight counts as *potential seeding capacity*, not realised passenger flow.

$$
F_{\text{DEC}} = \text{# CN/HK ➝ EU flights in Dec 2019},\quad
F_{\text{MAR}} = \text{# flights in Mar 2020},\quad
F_{\text{SUM}} = F_{\text{DEC}} + F_{\text{MAR}}
$$

A look-up table built from **OurAirports** (airport → ISO-2) plus EUROCONTROL’s own mapping (ISO-2 → ISO-3) assigns each flight to its destination country.

**Why fix exposure to these two months?**
*Temporal precedence.* Both snapshots precede the 5 May outcome, eliminating reverse causation (deaths → fewer flights).  Later traffic volumes would mix cause and effect and reflect domestic restrictions rather than initial import pressure.  Extending the panel with post-2020 exposure is left for future work.

*(Full R pipeline in `notebooks/02b_flight_opensky.R`; the processed tables are stored in `data/processed/flights_country.rds`.)*
