...
Because completeness is critical here, **all exposure metrics use ADRR**; OpenSky is retained only as a **sanity-check benchmark**, see [Coverage audit](#coverage-audit) and [Appendix A](#appendix-a).
...
Full details are in [Appendix A](#appendix-a) (Table A.1, Fig. A.1).
...
coverage_tbl |>
  ...
  head(20) |>
  knitr::kable(
    caption = "Excerpt of the coverage audit (top half of the table). All 41 rows are reproduced in Appendix A."
  )
```
...
| **Residual risk** | Absolute flight counts still diverged after March: a handful of hubs kept 200‑350 rotations, so a trickle of import potential remained (see Δ-flights column in [Table A.2](#appendix-a)). |
...
All four exposure metrics are positively associated with first-wave excess mortality (Table @tbl-spearman_2020). Using the **combined Dec + Mar count**:
...
* Figure @fig-scatter-2020 shows that the right-hand tail is driven by the big Western gateways (**DEU, GBR, NLD, FRA, ESP, ITA**).
...
knitr::kable(
  tbl_all,
  caption = "Spearman $\\rho$ between early flight exposure and cumulative excess mortality, 2020–2023."
)
...
**Pattern.**
The 2020 coefficient is significantly positive, but the association weakens in 2021 and turns **significantly negative** in 2022 (ρ = –0.46, *p* = 0.021) and remains negative in 2023 (ρ = –0.42, *p* = 0.037). Scaling by population (*Flights / million*) accelerates the flip: already negative and significant by 2021.
...
Controlling for the share of population ≥ 65 changes ρ by **≤ 0.02** in any year (Table @tbl-sensitivity). Age structure therefore cannot account for the sign-flip.

### Key take-away — the signal flips within a year

* ρ = **+0.53** in 2020  
* ρ ≈ 0 in 2021 (ns)  
* ρ ≈ **–0.45** and significant in 2022-23

## Main finding in one breath  

Among the 26 continental EUROCONTROL states, heavier direct **CN/HK → EU** traffic was associated with **substantially higher first-wave excess mortality** (Spearman ρ = 0.53, *p* = 0.007, *n* = 26).  
The rank-based link weakened during 2021 and reversed by 2022-23 (ρ ≈ –0.45, *n* = 25 after Ukraine’s exit).

## Why did the signal fade?

* **Wealth & structural vulnerability.** A 34-country panel shows excess-death percentages **fall** with GDP-per-capita (Pearson *r* ≈ –0.60; Spearman ρ ≈ –0.69) and **rise** with poverty and inequality [@ioannidis2023]. The four most connected hubs (DEU, GBR, NLD, FRA) sit in Europe’s top GDP tercile, helping to explain why the positive flight–mortality link disappears after 2020.

* **Vaccination rollout.** On **5 May 2022** the high-exposure quartile (**Q4**, *n* = 6) had a median **78 %** of their populations fully vaccinated versus **≈ 62 %** in the low-exposure quartile (**Q1**, *n* = 7) (OWID variable `people_fully_vaccinated_per_hundred` [@mathieu2021].)
...
```{r vacc_by_exposure_quartile, echo=FALSE}
corr_df |>
  dplyr::filter(target_date == as.Date("2020-05-05")) |>
  ...
  dplyr::group_by(q) |>
  dplyr::summarise(
    countries = paste(sort(iso3), collapse = ", "),
    .groups = "drop"
  ) |>
  ...
  knitr::kable(
    ...
    caption   = "Table A.3 Country membership of the flight-exposure quartiles \
(Dec 2019 + Mar 2020 baseline, *n* = 25; quartiles contain 6–7 countries each)."
  )
```

For the descriptive comparison in the Discussion we extracted `people_fully_vaccinated_per_hundred` from the OWID series, taking the observation nearest to **5 May 2022 (± 7 days)** for the same set of EUROCONTROL states (script `01b_extract_vax.R`, saved as `data/processed/vax_snapshot.{csv,rds}`).

These figures are *not* used in any regression or correlation test.