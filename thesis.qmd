...
[READ TO DO NEXT: (Also need 02b in bash script)]
...
Our own cross-check (Table [@tbl-os-vs-eu]) confirms the blind-spot problem: **five EUROCONTROL member states receive no China/HK arrivals in OpenSky at all**, and most low-volume countries show double-digit undercounts for December 2019.
...
Consequently, **all exposure metrics in this thesis are based on EUROCONTROL**. OpenSky is kept **solely as a quality-control benchmark** to illustrate why ADS-B alone would have understated early China–Europe connectivity (see [Appendix A](#appendix-a)).
...
EUROCONTROL snapshots include all commercial IFR traffic from mainland China (CN) and Hong Kong (HK). No Macao–Europe flights were recorded in either December 2019 or March 2020. Accordingly, all exposure figures refer to CN + HK; the full acronym CN/HK/MO is kept only on first use.
...
Table [@tbl-os-vs-eu] lists, for each EUROCONTROL state, the direct CN/HK flights captured by both sources in Dec 2019, Feb 2020, and Mar 2020.
...
euro_map  <- readr::read_csv("data/eurocontrol_iso_map.csv", show_col_types = FALSE)
