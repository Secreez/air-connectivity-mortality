...
)$iso3 |> setdiff(EXCLUDE_ISO3)

owid_path <- here::here("data/raw/owid/owid-covid-data.csv")

covid_raw <- readr::read_csv(owid_path, show_col_types = FALSE) |>
  dplyr::filter(iso_code %in% euro_ISO3)

ref_dates <- lubridate::ymd(c("2020-05-05", "2021-05-05", "2022-05-05", "2023-05-05"))

win_tbl <- purrr::map_dfr(
  ref_dates,
  \(d) tibble::tibble(target_date = d, date = seq(d - 7, d + 7, by = "days"))
)

owid_snapshots <- covid_raw |>
  dplyr::select(iso_code, location, date, excess_mortality_cumulative_per_million) |>
...
stopifnot(!"UKR" %in% owid_snapshots$iso_code)

readr::write_rds(owid_snapshots, here::here("data/processed/owid_excess_snapshots.rds"))
readr::write_csv(owid_snapshots, here::here("data/processed/owid_excess_snapshots.csv"))

message("✓ owid_excess_snapshots: ", nrow(owid_snapshots), " rows (UKR excluded ex ante)")
