...
col_subset <- c(
  "ECTRL ID", "ADEP", "ADES", "ADEP Latitude", "ADEP Longitude",
...
flights_country <- flights_filtered %>%
  ...
  mutate(
    across(
      starts_with("total_inbound_flights_"),
      ~ replace_na(.x, 0)
    ),
...
cat(
  "Defined CN/HK/MO airports:", total_defined_airports, "\n",
...
ggplot(
  ...
  labs(
    ...
    fill = NULL,
    caption = paste(
      "Total direct flights - Dec:", sum(plot_data$flights[plot_data$month == "Dec 2019"]),
...
adep_counts <- bind_rows(
  ...
  pivot_wider(
    names_from = month, # -> flights_dec19 / flights_mar20
    values_from = flights,
...
adep_usage %>%
  ...
  select(
    ADEP, iata_code, name, iso_country,
...
keep_countries <- analysis_df %>%
  count(iso3) %>%
  filter(n == 4) %>%
  pull(iso3) # keep only countries with all 4 snapshots
...
map_data <- europe_map %>%
  ...
  mutate(
    plot_fill = replace_na(total_inbound_flights_combined, 0),
    excluded = iso_a3 == "UKR"
  )
...
ggplot(
  ...
  labs(
    title = "Top-10 Eurocontrol destinations by direct CN/HK/MO flights",
