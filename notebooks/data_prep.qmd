...
plot_facets <- ggplot(plot_df,
  ...
  scale_y_continuous(labels = comma,
                     expand  = expansion(mult = c(.06, .10))) + # extra headroom
  labs(
  ...
  theme(
    ...
    plot.title.position = "plot",
    panel.spacing     = unit(2, "lines"),   # <-- more space between rows
...
# high‑res export
ggsave("scatter_facets.png", plot_facets, width = 7, height = 9, dpi = 300)
```
...
```{r}
## Base map of continental Eurocontrol members only
europe_map <- rnaturalearth::ne_countries(scale = "medium",
                                          returnclass = "sf") %>%
  filter(iso_a2 %in% eurocontrol_countries) # reuse the vector
...
map_data <- europe_map %>%
  ...
  mutate(
    plot_fill = ifelse(is.na(total_inbound_flights_combined), 0,
                       total_inbound_flights_combined),
    excluded  = iso_a3 == "UKR" # highlight Ukraine
  )

##  Plot
ggplot(map_data) +
  geom_sf(aes(fill = plot_fill), colour = "white", linewidth = 0.2) +
  geom_sf(data = subset(map_data, excluded), # hatched Ukraine
          fill  = "grey70", colour = "grey40", linewidth = 0.2,
          alpha = .4, linetype = "33") +
  scale_fill_viridis_c(
    option  = "plasma",
    trans   = "log10", # compress long tail (0 → treated separately)
...
# Few other Ideas for Analysis:

- Population-normalise flights (flights per million) - Which brings farier comparison for microstates like Luxembourg Malta depending on density of flights .. Ergo: mutate(flights_pm = total_inbound_flights_combiend / population_millions)

```{r}
pop_df <- read_csv("data/...")  # iso3, population (from owid)
analysis_df <- analysis_df %>%
  left_join(pop_df, by = "iso3") %>%
  mutate(
    flights_pm = total_inbound_flights_combined / (population / 1e6)
  )
```

Then: Re‑run spearman_by_month("flights_pm") to see if ρ stays positive in 2020.


- Add vax coverage / GDP per capita columns - Got enough time for it.. Showing visually that 2022-23 esp. mrotality aligns mroe with structural variables or atleast a trend. Ergo: left_join(ovid_wax, by = iso3)

```{r}
gdp <- read_csv("data/...")  # iso3, gdp_ppp_2020
vax <- read_csv("data/...")  # iso3, vax_share

analysis_df <- analysis_df %>%
  left_join(gdp, by = "iso3") %>%
  left_join(vax, by = "iso3")
```

- Cluster heat-map of z-scored flights + four-year mortality - To see if there are any clusters of countries that are similar in terms of flight exposure and mortality. This could help identify patterns or outliers. - pheatmap() (https://www.rdocumentation.org/packages/pheatmap/versions/1.0.12/topics/pheatmap) on scaled matrix.

- Is Excess_cum_mort really the data and not just an estimation?
