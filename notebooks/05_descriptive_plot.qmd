...
*In the Wording, China/Hong Kong/Macao (CN/HK/MO) refers to the three regions together. It should be noted, that no flights where recorded from Macao to Europe in the data set. The analysis is therefore based on flights from China and Hong Kong only.

In December 2019 the direct China/HK/Macao traffic into Europe was highly concentrated: Germany (DE), the United Kingdom (GB) and the Netherlands (NL) alone handled > 60 % of all arrivals, followed by France and Belgium as secondary hubs. Every other Eurocontrol state receiver fewer than 300 flights that month. By March 2020 volumes collapsed everywhere (blue bars) yet the rnaking hardly changed: the same three hubs still dominated, each retaining 30-50 % of their December traffic, while Italy, Turkey and Finland - already mid-tier in December - fell below 250 flights. Combning both months (green bars) therefore produces an extremely right-skewed distribution: the top two countries (DE, GB) each saw > 900 direct flights, four times as many as rank #4. Intepretation: epidemic "import pressure" from direct flights was not evenly spread across Europe; instead, a small group of Western European gateways bore the brunt of exposure in both the seeding phase (Dec 2019) and the pre-lockdown (? should I call it that?) phase (Mar 2020).
...
Direct links weere not ony concentrated by volume (see @bar_top10), but also by route diversity. In total, 15 Chinese/HK/Macao airports fed London-Heahtrow, 13 fed Paris-CDG and 12 fed Frankfurt between December 2019 and March 2020 (left panel, green bars). Every other Eurocontrol desitination received routes from ≤ 10 origins, and most from fewer than 6. The month-specific panel (right) reveals that this breadth of origins existed already in December (orange) and, despite the March collapse (blue), the major hubs still drew from 4-6 different airports while almost all secondary gateways fell to one or two. **Implication:** the first-wave seeding risk was amplified not just by sheer fligth counts but by the variety of source cities funnelling into the same Western European hubs, increasing the chance of multiple independent introductions.
...
```{r scatter_volume_vs_diversity, fig.height=5}
...
p_scatter <- ggplot(
  ...
  labs(
    ...
    subtitle = "Each dot represents one Eurocontrol destination airport",
    caption  = "Spearman ρ = 0.86 (p < 0.001); 38 destinations"
  ) +
...
```
* **Clear positive association.** Among the 38 EUROCONTROL airports that handled at least one direct flight from Chinese/HK/Macao in Dec 2019 + Mar 2020, flight volume and route diversity rise together (Spearman ρ ≈ 0.86, p < 0.001). *Rule of thumb:* every additional ~100 flights is accompanied by **≈ 1–2 extra origin airports**.
...
* **Cargo outlier – Liège (LGG).** Liège appears unusually diverse for its mid-range volume (205 / 7). This reflects its role as a **dedicated cargo hub** (UPS, DHL, Cainiao, …). ADRR treats cargo and passenger flights alike, so LGG’s position may **over-state passenger-borne seeding risk** [Need to research more about cargo overall.
* **Limitation.** The scatter uses **filed IFR flight plans**, not passenger counts, and weights each origin airport equally. Route diversity therefore proxies the *opportunity* for multiple introductions, not the number of infected travellers.
* **Link to country-level results.** The same right-skew underlies the country-level correlation (ρ ≈ 0.53 for “combined” exposure): national rankings are driven largely by how heavily these few hubs connect to China.
...
p_collapse <- ggplot(
  ...
  labs(
    title = "March collapse in direct CN-HK-MO → Eurocontrol flights",
...
knitr::kable(
  collapse_tbl,
  caption = "Direct CN-HK-MO passenger flights: Dec 2019 vs Mar 2020 \
(countries with ≥ 5 Dec flights)",
  align = c("l", "r", "r", "r", "r")
)
```
...
knitr::kable(
  top_tbl,
  caption = "Largest proportional drops / increases in direct CN-HK-MO flights \
(Mar 2020 vs Dec 2019, countries with ≥ 5 Dec flights)",
