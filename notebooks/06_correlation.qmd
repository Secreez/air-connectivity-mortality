...
We Report 95 % confidence limits for Spearman’s ρ from a **non‑parametric percentile
bootstrap (B = 5 000 resamples)**.

* **Minimum B.** Coverage‑error analyses show that at least 2 000
replicates are needed in small‑sample settings (DiCiccio & Efron 1996).  
* **Diminishing returns.** Empirical work on rank correlations finds that coverage
error keeps falling but flattens out once *B* reaches ≈ 4 000–5 000
(Tong et al. 2016; Mokhtar et al. 2023).  
* **Monte‑Carlo noise.** With *B* = 5 000, the Monte‑Carlo
s.e. of the 2.5 th quantile is  

$$\sqrt{0.025\,(1-0.025)/B}\;=\;0.007,$$

  well below the sampling variability of ρ for *n* ≈ 25.  
* **Cost.** Five thousand resamples run in x s on a PC, so the
extra precision is essentially free.
* **Reproducibility.** We fix the random seed (`set.seed(42)`), making the
intervals byte‑for‑byte reproducible.

Hence 5 000 draws strike a pragmatic balance between statistical
