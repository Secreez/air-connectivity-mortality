...
age_df <- read_csv(
  ...
  slice_max(date, with_ties = FALSE) |>
  ungroup() |>
  transmute(iso3   = iso_code,
            aged65 = aged_65_older)

corr_df <- corr_df |> left_join(age_df, by = "iso3")
stopifnot(!anyNA(corr_df$aged65))
```
...
```{r save-spearman_res, echo=FALSE}
saveRDS(spearman_res,
        here::here("data/derived/spearman_res.rds"))
saveRDS(corr_df,
        here::here("data/derived/corr_df.rds"))
```
...
low_med  <- median(snap20$excess_mortality_cumulative_per_million[
                     snap20$total_inbound_flights_combined <= q[1] ])
high_med <- median(snap20$excess_mortality_cumulative_per_million[
                     snap20$total_inbound_flights_combined >= q[2] ])

abs_gap  <- round(high_med - low_med)
ratio    <- round(abs(high_med) / abs(low_med), 1)

cat("Absolute gap top vs bottom quartile (2020):",
    abs_gap, "deaths / million\n")
cat("Magnitude ratio:", ratio, "×\n")
```
...
## Confounder Test (Sanity Check)

For Results > Sensitivity 
- Partial Spearman or simple multiple rank regresion
- using age 65+ share or GDP pc / Vacc (Think about it)
- Just one sentence in the text like Adding median age lowers ...
- Just to be safe against confounding

```{r confounder_test, echo = FALSE}
partial_by_year <- corr_df |>
  ...
  dplyr::group_modify(~{
    ...
    dd <- dplyr::filter(
      .x,
      complete.cases(total_inbound_flights_combined,
                     excess_mortality_cumulative_per_million,
                     aged65)
    )

    n <- nrow(dd)
    if (n < 3 || sd(dd$aged65) == 0) {
      return(tibble())
    }
    ...
    tibble(
      year = lubridate::year(.y$target_date),
      rho_baseline = cor(dd$total_inbound_flights_combined,
                         dd$excess_mortality_cumulative_per_million,
                         method = "spearman"),
      rho_adj_age  = p$estimate,
      p_adj_age    = p$p.value
    )
  }) |>
  dplyr::ungroup()
...
knitr::kable(
  partial_by_year |> dplyr::mutate(across(where(is.numeric), round, 3)),
  caption = "Spearman ρ with and without adjustment for share ≥ 65, by year."
)


```

```{r save-partial_by_year, echo=FALSE}
saveRDS(partial_by_year,
        here::here("data/derived/partial_by_year.rds"))
```

Controlling for the share of population ≥ 65 alters Spearman ρ by < 0.02 in any year (Table X), confirming that age structure is not driving the observed pattern.
...
Our metrics count every filed ICAO type *S* / *N* plan equally.  They cannot
distinguish well-loaded passenger legs from cargo-only or slot-retention
“ghost” flights, which inflates **human** exposure at freight-oriented
airports.

- At classic hubs passenger traffic collapsed in 2020 (Frankfurt **–73.4 %** pax, 18.8 m) while cargo slipped only **–8.3 %**, thanks to ≈ **8 600 “preighter” rotations**; pure freighters performed about **81 %** of all cargo movements.  
- Liège, a dedicated cargo airport, **grew to 1.12 Mt (+24 %)** and ranked **#6** in Europe.  
- Our flight counts therefore over-state person-to-person seeding pressure wherever cargo or ghost operations dominate [@fraport2021].

A second, structural caveat is that EUROCONTROL releases only four “snapshot” months (Mar | Jun | Sep | Dec) per year; February 2020—the critical month between Wuhan’s lockdown and Europe’s first local clusters—is missing.
...
**Two-month window (Dec 2019 & Mar 2020).** EUROCONTROL's public archive releases only four sample months per year; February—a critical seeding month—is missing [@eurocontrol2025].
...
exposure_q <- corr_df |>                             
  filter(target_date == as.Date("2020-05-05")) |>     
  mutate(q = ntile(total_inbound_flights_combined, 4)) |> 
  dplyr::select(iso3, q)
...
snap <- dplyr::left_join(exposure_q, vacc22, by = "iso3")

snap |> 
  group_by(q) |> 
  summarise(median_vacc = median(vacc, na.rm = TRUE))

```

```{r save-result_tbl, echo=FALSE}
saveRDS(result_tbl,
        here::here("data/derived/rho_restricted_2020.rds"))
```