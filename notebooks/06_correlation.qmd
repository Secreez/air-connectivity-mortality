...
* **Flight-exposure proxy (filed IFR plans).**  
  We count ICAO flight types **S** (scheduled) and **N** (non-scheduled commercial) from EUROCONTROL.  
  – Actual passenger loads are unknown; “ghost” and cargo-only flights are included.  
  – Multi-leg itineraries via third-country hubs (e.g. DXB, DOH) are excluded, so exposure is *under-estimated* for transfer states and *over-estimated* for cargo hubs such as Liège [@strohmeier2021].
...
* **Excess-mortality uncertainty.**  
  Outcome data merge the World Mortality Dataset with HMD-STMF. National registration lags, baseline-fit choices, and wartime disruption (Ukraine ≥ 2022) widen credible intervals [@karlinsky2021; @msemburi2023].  
...
* **Ecological, country-level design.**  
  Aggregation hides sub-national heterogeneity (e.g. Lombardy vs Sicily) and invites the ecological fallacy.
...
* **Residual confounding.**  
  No adjustment for age structure, NPIs, vaccination, or health-system capacity—factors known to affect COVID-19 mortality and correlate with air connectivity [@ioannidis2023].
