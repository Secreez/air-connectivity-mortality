...
write_csv(pop_comp, file.path(PROC_DIR, "pop_comparison_2020.csv"))
saveRDS(pop_comp, file.path(DER_DIR,  "pop_comparison_2020.rds"))

big_gaps <- pop_comp |>
  filter(abs(rel_diff) > DIFF_LIMIT) |>
  mutate(pct = scales::percent(rel_diff, accuracy = 0.1))

write_csv(big_gaps, file.path(DER_DIR, "pop_big_gaps_2020.csv"))
