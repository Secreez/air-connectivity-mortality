...
POP_CUT    <- 2e6  # micro-state threshold used in 06_correlation
DIFF_LIMIT <- 0.10  # 10 % OWID–WPP alarm line
OUT_DIR    <- here::here("data", "processed")
dir.create(OUT_DIR, showWarnings = FALSE, recursive = TRUE)

# WPP 2020
data(pop1dt, package = "wpp2024") # ‘000 persons’
pop_wpp20 <- pop1dt |>
  filter(year == 2019) |> # mid-2020 = 31-Dec-2019
  mutate(
    iso3 = countrycode(
...
# micro-states (2 million threshold ... for 06_correlation ~ sensitivity)
small_in_baseline <- pop_comp |> 
  filter(pop_owid < POP_CUT) |> 
  arrange(pop_owid)

readr::write_csv(
  small_in_baseline,
  here::here("data", "processed", "pop_below2M.csv")
)

small_iso <- small_in_baseline$iso3
